...
const ReceiptDownload = ({calculateTotalCost, shippingValue, cartItems, shippingOption, setShippingOption, couponCode, totalCost}) => {
  const { data: session } = useSession();

  const generateReceipt= () =>{
    return `
    <div className='bg-white font-mono px-4 py-4 text-sm'>
     ...
     <div className='fixed w-full min-h-screen backdrop-blur top-0 left-0 z-50 flex justify-center items-center '>
        <div className='md:w-1/4 max-h-screen bg-white font-mono px-4 py-4 text-sm overflow-y-scroll'>
          ...
          <table className='mt-4 w-full'>
            ...
            <tr>
              <td>Coupon Code</td>
              <td className='text-right'>{couponCode}</td>
            </tr>
            <tr>
              <td>Total</td>
              <td className='text-right'>${calculateTotalCost}</td>
          ...
          <Link href='/' 
            onClick={handleHome}
            className='bg-black py-1 px-2 text-white'> &larr; Home</Link> 
    </div>
    </div>
  );
};
